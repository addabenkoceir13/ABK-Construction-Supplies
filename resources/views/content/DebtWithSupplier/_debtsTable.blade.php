  ...
  <table id="datatable-debt" class="table table-hover align-middle mb-0">
    <thead class="table-light">
      <tr>
        ...
        <th><i class="bx bx-truck me-1 text-primary"></i>{{ $tractorDriverLabel ?? __('Supplier') }}</th>
    ...
    <tbody>
      @forelse ($debts as $debt)
        <tr data-row-id="{{ $debt->id }}">
          ...
          <td>
            @if ($debt->tractorDriver)
              <span class="badge bg-label-info rounded-pill px-3 py-2">
                <i class="bx bx-user-check me-1"></i>{{ $debt->tractorDriver->fullname }}
              </span>
            @else
              <span class="badge bg-label-secondary rounded-pill px-2 py-1">-</span>
            @endif
          </td>
          <td>
          ...
          <td class="text-center">
            <div class="d-flex align-items-center justify-content-center gap-1">
              <a href="{{ route('debt-supplier.show', $debt->id) }}" class="btn btn-icon btn-sm btn-outline-info" data-bs-toggle="tooltip" title="{{ __('View Debt') }}">
                <i class="bx bx-show"></i>
              </a>
              <a href="{{ route('debt-supplier.edit', $debt->id) }}" class="btn btn-icon btn-sm btn-outline-success" data-bs-toggle="tooltip" title="{{ __('Modify Debt') }}">
