...
@section('title', __('Modify Supplier Debt') . ' - ' . $debt->fullname)

@section('content')
<div class="d-flex flex-wrap justify-content-between align-items-center mb-4">
  <div>
    <h4 class="fw-bold mb-1">
      <span class="text-muted fw-light"><a href="{{ route('debt-supplier.index') }}" class="text-muted text-decoration-none">{{ __('Supplier Debts') }}</a> /</span> {{ __('Modify Supplier Debt') }}
    </h4>
    <p class="text-muted mb-0 small">{{ __('Update supplier delivery debt dossier, note, delivered products, and amounts.') }}</p>
  </div>
  <div class="d-flex gap-2 mt-2 mt-sm-0">
    <a href="{{ route('debt-supplier.show', $debt->id) }}" class="btn btn-outline-primary d-flex align-items-center gap-1 shadow-sm">
...
<form action="{{ route('debt-supplier.update', $debt->id) }}" method="POST" enctype="multipart/form-data">
  ...
  <!-- 1. Supplier & Customer Information Card -->
  <div class="card shadow-sm border-0 mb-4">
    ...
    <div class="card-body pt-4">
      <div class="row g-3">
        <div class="col-12">
          <div class="alert alert-primary bg-label-primary border-0 rounded-3 p-3 mb-2 d-flex align-items-center gap-2">
            <i class="bx bxs-truck fs-4 text-primary"></i>
            <div>
              <span class="small text-muted fw-semibold d-block">{{ __('Tractor Driver / Supplier') }}</span>
              <strong class="text-primary">{{ $debt->tractorDriver->fullname ?? '-' }}</strong>
            </div>
          </div>
        </div>
        <div class="col-md-6">
  ...
  <div class="card shadow-sm border-0 mb-4">
    <div class="card-header bg-transparent border-bottom py-3 d-flex justify-content-between align-items-center">
      <div class="d-flex align-items-center gap-2">
        ...
        <h6 class="card-title mb-0 fw-semibold">{{ __('Delivered Products & Materials') }}</h6>
      </div>
    ...
    <div class="card-footer bg-transparent border-top py-3 d-flex justify-content-end gap-2">
      <a href="{{ route('debt-supplier.index') }}" class="btn btn-outline-secondary">{{ __('Cancel') }}</a>
