...
<div class="modal fade" id="modalAddDebt" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
  <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable" role="document" style="max-width: 1140px;">
    <div class="modal-content shadow-2xl border-0 overflow-hidden custom-debt-modal {{ Session::get('theme') == 'dark' ? 'theme-dark' : 'theme-light' }}">
      <form action="{{ route('debt.store') }}" method="POST" id="formAddDebt" class="needs-validation d-flex flex-column h-100 w-100 m-0">
        @csrf
        <input type="hidden" name="tractor_driver_id" value="{{ $supplier->id ?? '' }}">

        {{-- Modal Top Accent Bar --}}
        <div class="modal-top-accent flex-shrink-0"></div>

        {{-- Modal Header --}}
        <div class="modal-header border-bottom py-3 px-4 flex-shrink-0">
          <div class="d-flex align-items-center gap-3">
            <div class="avatar avatar-md bg-label-primary rounded-3 d-flex align-items-center justify-content-center shadow-xs modal-header-icon">
              <i class="bx bx-user-plus fs-3"></i>
            </div>
            <div>
              <div class="d-flex align-items-center gap-2">
                <h5 class="modal-title mb-0 fw-bold text-heading">{{ __('إضافة سجل دين جديد') }}</h5>
                <span class="badge bg-label-primary rounded-pill px-2 py-1 fs-tiny product-count-badge">
                  <i class="bx bx-package me-1"></i><span id="modal-product-counter">1</span> {{ __('سلعة') }}
                </span>
              </div>
              <small class="text-muted">{{ __('تسجيل وتوثيق تفاصيل دين الزبون والمواد المشتراة بدقة') }}</small>
            </div>
          </div>
        ...
        <div class="modal-body p-4 custom-modal-body flex-grow-1 overflow-auto">
          ...
          <div class="section-card customer-section-card mb-4 p-3 rounded-3 border shadow-2xs">
            <div class="d-flex align-items-center gap-2 mb-3 text-primary">
              ...
              <h6 class="mb-0 fw-bold text-uppercase fs-tiny tracking-wider">{{ __('معلومات الزبون والمعاملة') }}</h6>
            </div>
            
            <div class="row g-3">
              {{-- Customer Name --}}
              <div class="col-md-6 col-lg-3">
                <label for="fullname-search" class="form-label text-heading small fw-bold mb-1">
                  {{ __('اسم الزبون') }} <span class="text-danger">*</span>
                </label>
                <div class="input-group input-group-merge custom-input-group">
                  <span class="input-group-text border-end-0"><i class="bx bx-user text-primary"></i></span>
                  <input type="text" id="fullname-search" name="fullname" class="form-control border-start-0 ps-1 @error('fullname') is-invalid @enderror" placeholder="{{ __('اكتب الاسم أو اختر من القائمة...') }}" autocomplete="off" list="listFullName" required />
              ...
              <div class="col-md-6 col-lg-3">
                ...
                <div class="input-group input-group-merge custom-input-group">
                  <span class="input-group-text border-end-0"><i class="bx bx-phone text-primary"></i></span>
              ...
              <div class="col-md-6 col-lg-3">
                ...
                <div class="input-group input-group-merge custom-input-group">
                  <span class="input-group-text border-end-0"><i class='bx bx-calendar-check text-primary'></i></span>
              ...
              <div class="col-md-6 col-lg-3">
                ...
                <div class="input-group input-group-merge custom-input-group">
                  ...
                  <input type="text" name="note" id="note" class="form-control border-start-0 ps-1" placeholder="{{ __('ملاحظة اختيارية...') }}" />
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="d-flex flex-wrap justify-content-between align-items-center mb-3">
            <div class="d-flex align-items-center gap-2">
              <div class="badge bg-label-primary p-2 rounded-3"><i class="bx bx-cart-alt text-primary fs-5"></i></div>
              <div>
                <h6 class="mb-0 fw-bold text-heading">{{ __('السلع والمواد المشتراة') }}</h6>
                <small class="text-muted">{{ __('حدد المنتوج، الكمية، والمبلغ لكل مادة') }}</small>
              </div>
            </div>
            <button type="button" id="add-product-create" class="btn btn-sm btn-outline-primary d-inline-flex align-items-center gap-1 add-item-btn shadow-xs">
            ...
            <div class="product-row-create product-item-card p-3 rounded-3 border position-relative" data-row-index="1">
              <div class="d-flex justify-content-between align-items-center mb-2 pb-2 border-bottom border-bottom-divider">
                <span class="badge rounded-pill px-2 py-1 fs-tiny fw-semibold item-badge-num">
                  <i class="bx bx-cube-alt text-primary me-1"></i>{{ __('المادة رقم') }} <span class="row-num">1</span>
                </span>
        ...
        <div class="modal-footer border-top py-3 px-4 flex-shrink-0 d-flex justify-content-between align-items-center custom-modal-footer">
          ...
          <button type="submit" class="btn btn-primary px-5 py-2 shadow-sm d-flex align-items-center gap-2 fw-semibold save-debt-submit-btn">
...
<style>
  ...
  .add-item-btn:hover {
    ...
    box-shadow: 0 4px 10px rgba(105, 108, 255, 0.25) !important;
  }
  ...
  .custom-debt-modal.theme-dark .live-total-preview-card {
    background: linear-gradient(135deg, rgba(113, 221, 55, 0.12) 0%, rgba(105, 108, 255, 0.12) 100%) !important;
