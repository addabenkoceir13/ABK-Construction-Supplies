    ...
    <div class="modal-content shadow-lg border-0">
      <div class="modal-header bg-transparent border-bottom pb-3">
        <div class="d-flex align-items-center gap-2">
          ...
          <h5 class="modal-title mb-0 fw-semibold">{{ __('Delete Debt Record') }}</h5>
        </div>
      ...
      <form action="{{ route('debt.destroy', $debt->id) }}" method="POST">
        ...
        <div class="modal-body py-4">
          <div class="d-flex align-items-start gap-3">
            ...
            <div>
              <h6 class="mb-1">{{ __('Are you sure you want to delete this debt?') }}</h6>
              <p class="text-muted small mb-0">{{ __('Customer:') }} <strong class="text-danger">{{ $debt->fullname }}</strong> | {{ __('Total Debt:') }} <strong>{{ number_format($debt->total_debt_amount, 2) }} DZ</strong>. {{ __('This will remove all associated products and payment histories.') }}</p>
            </div>
          </div>
        </div>
