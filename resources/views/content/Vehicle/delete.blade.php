    ...
    <div class="modal-content shadow-lg border-0">
      <div class="modal-header bg-transparent border-bottom pb-3">
        <div class="d-flex align-items-center gap-2">
          ...
          <h5 class="modal-title mb-0 fw-semibold">{{ __('Delete Vehicle') }}</h5>
        </div>
      ...
      <form action="{{ route('services.vehicle.destroy', $vehicle->id) }}" method="POST">
        ...
        <div class="modal-body py-4">
          <div class="d-flex align-items-start gap-3">
            ...
            <div>
              <h6 class="mb-1">{{ __('Do you really want to delete this vehicle?') }}</h6>
              <p class="text-muted small mb-0">{{ __('Vehicle:') }} <strong class="text-danger">{{ $vehicle->name }}</strong> ({{ $vehicle->license_plate }}). {{ __('This action cannot be undone.') }}</p>
            </div>
          </div>
        </div>
