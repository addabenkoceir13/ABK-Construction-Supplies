    ...
    <tbody>
        @foreach ($fuelStations as $fuelStation)
            <tr>
                ...
                <td>
                  <input type="checkbox" class="row-checkbox" name="ids[]" value="{{ $fuelStation->id }}">
                </td>
                <td>
                ...
                <td>
                    <a href="javascript:void(0);" data-bs-toggle="modal"
                        data-bs-target="#modalEditFuelStation-{{ $fuelStation->id }}">
                        <span class="badge bg-label-success" data-bs-toggle="tooltip" data-bs-offset="0,4"
                            data-bs-placement="top" data-bs-html="true"
                            title="<i class='bx bx-bell bx-xs' ></i> <span>{{ __('Edit Fuel Receipt') }}</span>">
                            <i class="bx bx-edit-alt me-1"></i></span>
                    </a>
                    @include('content.Fuelstation.edit')
                    <a href="javascript:void(0);" data-bs-toggle="modal"
                        data-bs-target="#modalDeleteFuelStation-{{ $fuelStation->id }}">
                        <span class="badge bg-label-danger" data-bs-toggle="tooltip" data-bs-offset="0,4"
                            data-bs-placement="top" data-bs-html="true"
                            title="<i class='bx bx-bell bx-xs' ></i> <span>{{ __('Delete Fuel Receipt') }}</span>">
                            <i class="bx bx-trash me-1"></i></span>
                    </a>
                    @include('content.Fuelstation.delete')
                    <a href="javascript:void(0);" data-bs-toggle="modal"
                        data-bs-target="#modalPaidedFuelStation-{{ $fuelStation->id }}"
                        data-row-id="{{ $fuelStation->id }}">
                        <span class="badge bg-label-primary" data-bs-toggle="tooltip" data-bs-offset="0,4"
                            data-bs-placement="top" data-bs-html="true"
                            title="<i class='bx bx-bell bx-xs' ></i> <span>{{ __('Pay a debt') }}</span>">
