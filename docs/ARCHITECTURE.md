...
As-built, not as-intended. Every claim below is tagged `[CONFIRMED path:line]` (read directly) or
`[ASSUMED]` (inferred, not provable by static reading alone). Where the two disagree, trust `[CONFIRMED]`
and treat this document, not the original design intent, as the map of the system you're actually working in.

Laravel 9 / PHP 8.0.2+ (constraint; the dev box runs PHP 8.3.29 — see `docs/README.md`). 19 controller files,
10 models, 9 repository interface+impl pairs, ~60 Blade templates. `app/` contains exactly six directories:
`Console, Exceptions, Http, Models, Providers, Repositories` `[CONFIRMED — ls app/]`. There is no `Services`,
`Actions`, `Jobs`, `Events`, `Listeners`, `Policies`, `Observers`, `DTO`, or `Http/Requests` directory anywhere
in the codebase.

**Contents:** [1. Pattern in use](#1-pattern-in-use) · [2. Layer boundaries](#2-layer-boundaries--the-real-dependency-graph)
· [3. Module coupling map](#3-module-coupling-map) · [4. Where state lives](#4-where-state-lives)
· [5. Consistency scores](#5-consistency-scores-per-convention) · [6. Load-bearing walls](#6-the-five-load-bearing-walls)
· [7. Open questions](#7-open-questions-not-resolvable-statically)

---

## 1. Pattern in use

The team's stated pattern is **Repository**. That's half-true: a repository *layer* exists and is wired, but
it's a passthrough DAO with no domain semantics, and four live request handlers plus three route closures
bypass it entirely.
...
This is the more honest axis. The repository layer holds almost no logic; it's a thin `Model::` wrapper.
...
| **V15** | Model coupled to an external search service; repository branches on it | `use ... Searchable` + `toSearchableArray()` `[CONFIRMED app/Models/FuelStation.php:13,38-49]`; `EloquentFuelStation::paginate` switches to `FuelStation::search()` when `$search` is set `[CONFIRMED :72-82]` | `config/scout.php:19` defaults to **`algolia`** `[CONFIRMED]`, though `.env.example:52` ships `SCOUT_DRIVER=database` — see `docs/README.md`. If a deployed `.env` leaves `SCOUT_DRIVER` unset, every fuel-station search hits a third party — and that branch **silently drops the `$start_date`/`$end_date` filters** while still `appends()`-ing them to the pagination links `[CONFIRMED :73-82]`. |
...
| **Scout / Algolia index** | A **second copy** of `fuel_stations` (7 fields) | `env('SCOUT_DRIVER','algolia')` in `config/scout.php:19`; `.env.example:52` ships `database` | Written implicitly by `Searchable`'s model observers; read by `EloquentFuelStation::paginate` `[CONFIRMED :75]`. Whether a real deploy diverges from `.env.example` and actually mirrors to Algolia is unverified — see `docs/README.md` "Search". |
...
`[CONFIRMED database/migrations/2025_08_18_144113_create_debt_histories_table.php:19]` — the history table
silently caps at 999,999.99.
...
Scored 0–5. 5 = one convention, applied everywhere. 0 = the convention does not exist. Full de-facto rules
(what to actually follow when writing new code) are in [`CONVENTIONS.md`](./CONVENTIONS.md); this table is
the scorecard, that document is the rulebook.
...
Ranked by blast radius if moved, renamed, or removed. These are the files a new contributor should read
before touching *anything* adjacent to them.
...
Worth naming even though the rest of this section covers classes. The client/supplier split — the primary
business axis of this application — is not a type, an enum, or a column. It is **one row in `tractor_drivers`**,
...
- Is `SCOUT_DRIVER` set to `database`/`null` in production, or is `fuel_stations` genuinely mirrored to
  Algolia? `config/scout.php:19` defaults to `algolia`, `.env.example:52` ships `database`, and no deployed
  `.env` is readable. Determines whether §4's "second source of truth" is real. `[ASSUMED unresolved]`
